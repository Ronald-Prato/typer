...
const EVALUATOR_DISABLED_MESSAGE =
  "El evaluador de find-the-bug está pausado hasta contar con un sandbox seguro.";
...
console.log("Total:", calculateTotal(items));`,
  ...
  const [currentError, setCurrentError] = useState<string>(
    EVALUATOR_DISABLED_MESSAGE
  );
  const [isMac, setIsMac] = useState(false);
  const [showSuccess, setShowSuccess] = useState(false);
  const [showError, setShowError] = useState(false);

  // Detect platform for shortcut display without executing user code.
  useEffect(() => {
  ...
  const checkCode = useCallback(async () => {
    setIsChecking(true);
    setCurrentError(EVALUATOR_DISABLED_MESSAGE);
    ...
    onError?.(EVALUATOR_DISABLED_MESSAGE);
  }, [onError]);
